            ...
            <table class="table w-full">
                <thead>
                    <tr class="bg-base-200/50">
                        <th class="py-3 px-4 cursor-pointer hover:bg-base-200 transition-colors select-none" wire:click="sortByColumn('id')">
                            <div class="flex items-center gap-1">
                                #
                                @if($sortBy['column'] === 'id')
                                    <x-icon name="o-chevron-{{ $sortBy['direction'] === 'asc' ? 'up' : 'down' }}" class="w-4 h-4 text-primary" />
                                @else
                        ...
                        <th class="py-3 px-4 cursor-pointer hover:bg-base-200 transition-colors select-none" wire:click="sortByColumn('overall_evaluation')">
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        @if($earnedBadge)
                                            @if($earnedBadge->getFirstMediaUrl('image'))
                                                <img src="{{ $earnedBadge->getFirstMediaUrl('image') }}" class="w-8 h-8 object-cover rounded-full shadow-sm" alt="{{ $earnedBadge->name }}" title="{{ $earnedBadge->name }}" />
                                            @else
                                                <x-icon name="s-star" class="w-8 h-8" style="color: {{ $earnedBadge->color_hex }}" title="{{ $earnedBadge->name }}" />
                                            @endif
                                        @endif
