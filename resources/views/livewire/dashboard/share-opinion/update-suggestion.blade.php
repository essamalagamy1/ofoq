<div class="max-w-4xl mx-auto">
    <x-header title="{{ __('lang.update_suggestion') ?? 'تعديل المقترح' }}" separator>
        <x-slot:actions>
        ...
        <x-form wire:submit="submit">
            ...
            <x-slot:actions>
                <div class="flex justify-end gap-3 mt-6">
                    <x-button label="{{ __('lang.cancel') ?? 'إلغاء' }}" class="btn-ghost" link="{{ route('parent.opinion') }}" />
                    <x-button label="{{ __('lang.save_changes') ?? 'حفظ التعديلات' }}" type="submit" class="btn-primary" spinner="submit" icon="o-check" />
                </div>
            </x-slot:actions>
        </x-form>
    </div>
