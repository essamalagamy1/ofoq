...
<body class="min-h-screen flex flex-col font-sans antialiased bg-base-200">
    <x-toast position="toast-top toast-{{ app()->getLocale() === 'ar' ? 'start' : 'end' }}" />
    ...
    <x-nav sticky full-width>
        ...
        <x-slot:actions class="!gap-2">
			...
			<label tabindex="0" class="btn btn-ghost btn-circle  btn-sm">
				<x-icon name="o-language" class="w-5 h-5 sm:w-6 sm:h-6" />
			</label>
            ...
            <div class="dropdown dropdown-end">
                ...
                <ul tabindex="0"
                    class="menu menu-sm dropdown-content bg-base-100 rounded-xl shadow-lg mt-4 p-3 w-auto min-w-[10rem] z-[999]">
                    <li>
                    ...
                    <li>
                        <a onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
                            style="cursor: pointer" class="flex items-center gap-2 whitespace-nowrap">
            ...
            <div class="flex-grow">
                @if (isset($breadcrumbs))
                    <x-ui.breadcrumb :items="$breadcrumbs"
                        ...
                        class="bg-base-300 p-3 rounded-box mb-3" icon-class="dark:text-white w-4 h-4"
                        link-item-class="text-sm font-bold" />
                @endif
                @session('error')
                    <x-alert title="{{ session('error') }}" icon="o-exclamation-triangle"
                        class="alert-warning mb-3 text-white"></x-alert>
                @endsession
                {{ $slot }}
            </div>
